...
static pid_t executeCgi(Connection& c) {
	...
	std::string filePath;
	if (c.req.getPath() == c.loc->path) {
		filePath = c.loc->root + "/" + c.loc->index;
	} else {
...
HTTPResponse RequestHandler::doneCgi(Connection& c) {
...
void RequestHandler::doneWritingCgi(Connection& c) {
	if (c.cgiReadFd != -1) {
		close(c.cgiReadFd);
		c.cgiReadFd = -1;
	}
